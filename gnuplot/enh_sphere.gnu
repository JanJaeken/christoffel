...
splot "slow_secondary.dat" u (sin($1)*cos($2)):(sin($1)*sin($2)):(cos($1)):(log10($17)) w pm3d notitle, "slow_secondary.dat" u (-sin($1)*cos($2)):(-sin($1)*sin($2)):(-cos($1)):(log10($17)) w pm3d notitle;
set title "Fast Secondary"
splot "fast_secondary.dat" u (sin($1)*cos($2)):(sin($1)*sin($2)):(cos($1)):(log10($17)) w pm3d notitle, "fast_secondary.dat" u (-sin($1)*cos($2)):(-sin($1)*sin($2)):(-cos($1)):(log10($17)) w pm3d notitle;

#Manually edit this range too!
set cbrange[-0.6:1.2]
set title "Primary"
splot "primary.dat" u (sin($1)*cos($2)):(sin($1)*sin($2)):(cos($1)):(log10($17)) w pm3d notitle, "primary.dat" u (-sin($1)*cos($2)):(-sin($1)*sin($2)):(-cos($1)):(log10($17)) w pm3d notitle;
