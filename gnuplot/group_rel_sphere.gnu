...
splot "slow_secondary.dat" u (sin($1)*cos($2)):(sin($1)*sin($2)):(cos($1)):12 w pm3d notitle, "slow_secondary.dat" u (-sin($1)*cos($2)):(-sin($1)*sin($2)):(-cos($1)):12 w pm3d notitle;
set title "Fast Secondary"
splot "fast_secondary.dat" u (sin($1)*cos($2)):(sin($1)*sin($2)):(cos($1)):12 w pm3d notitle, "fast_secondary.dat" u (-sin($1)*cos($2)):(-sin($1)*sin($2)):(-cos($1)):12 w pm3d notitle;
set title "Primary"
set cbrange [-P_max:P_max]
splot "primary.dat" u (sin($1)*cos($2)):(sin($1)*sin($2)):(cos($1)):12 w pm3d notitle, "primary.dat" u (-sin($1)*cos($2)):(-sin($1)*sin($2)):(-cos($1)):12 w pm3d notitle;
unset multiplot
unset output
